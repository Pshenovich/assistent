"""Per-user Yandex Disk OAuth2 (authorization code). Общий модуль для bot.py и usage_server.py."""
...
STATE_TTL_SEC = int(os.getenv("YANDEX_DISK_OAUTH_STATE_TTL_SEC", "900") or "900")
TOKEN_EXPIRY_SKEW_SEC = int(os.getenv("YANDEX_DISK_TOKEN_EXPIRY_SKEW_SEC", "120") or "120")

_REAUTH_MSG = "Выполните /yandex_disk_auth и заново подключите Яндекс Диск."
...
def _tokens_dir() -> Path:
    raw = os.getenv("YANDEX_DISK_USER_TOKENS_DIR", "").strip()
    if raw:
        p = Path(raw).expanduser()
        if not p.is_absolute():
            p = (HERE / p).resolve()
    else:
        p = HERE / "yandex_disk_user_tokens"
    p.mkdir(parents=True, exist_ok=True)
    return p


def _pending_dir() -> Path:
    raw = os.getenv("YANDEX_DISK_OAUTH_PENDING_DIR", "").strip()
    if raw:
        p = Path(raw).expanduser()
        if not p.is_absolute():
            p = (HERE / p).resolve()
    else:
        p = HERE / "yandex_disk_oauth_pending"
    p.mkdir(parents=True, exist_ok=True)
    return p
...
def _read_token_store(telegram_user_id: int) -> dict[str, object] | None:
...
def token_granted_scopes(telegram_user_id: int) -> set[str]:
    store = _read_token_store(telegram_user_id)
    if not store:
        return set()
    scope = str(store.get("scope") or "").strip()
    if not scope:
        return set()
    return {part for part in scope.split() if part}


def needs_scope_refresh(telegram_user_id: int) -> bool:
    """Токен подключён, но выдан без cloud_api:disk.read — ссылку на папку получить нельзя."""
    if not has_connection(int(telegram_user_id)):
        return False
    granted = token_granted_scopes(int(telegram_user_id))
    if not granted:
        return True
    return "cloud_api:disk.read" not in granted


def client_id() -> str:
    v = _env_first("YANDEX_DISK_OAUTH_CLIENT_ID", "YANDEX_OAUTH_CLIENT_ID")
    if not v:
        raise RuntimeError("Не задан YANDEX_DISK_OAUTH_CLIENT_ID в .env")
    return v


def client_secret() -> str:
    v = _env_first("YANDEX_DISK_OAUTH_CLIENT_SECRET", "YANDEX_OAUTH_CLIENT_SECRET")
    if not v:
        raise RuntimeError("Не задан YANDEX_DISK_OAUTH_CLIENT_SECRET в .env")
    return v


def redirect_uri() -> str:
    u = _env_first("YANDEX_DISK_OAUTH_REDIRECT_URI", "YANDEX_REDIRECT_URI")
    if not u:
        raise RuntimeError(
            "Не задан YANDEX_DISK_OAUTH_REDIRECT_URI (или YANDEX_REDIRECT_URI) — "
            "как в oauth.yandex.ru → Redirect URI."
        )
    return u
...
def oauth_scope() -> str:
    raw = (
        os.getenv(
            "YANDEX_DISK_OAUTH_SCOPE",
            "cloud_api:disk.read cloud_api:disk.write",
        ).strip()
        or "cloud_api:disk.read cloud_api:disk.write"
    )
    return raw
...
def _save_token_for_user(telegram_user_id: int, body: dict[str, object]) -> int:
    ...
    try:
        ...
        print(f"[yandex_disk_oauth] profile fetch failed uid={uid}: {e!r}")
...
def build_authorization_url(state: str) -> str:
    ...
    if not pending_path.exists():
        raise RuntimeError(
            "Сессия OAuth не найдена. Запросите ссылку снова: /yandex_disk_auth"
        )
...
def fetch_user_profile(access_token: str) -> dict[str, str]:
    """Возвращает login/email/display_name из Yandex Passport."""
    try:
        r = requests.get(
...
def exchange_code_and_save_token(code: str, state: str) -> int:
    ...
    if parsed is None:
        raise ValueError(
            "Сессия авторизации устарела или неверная. Запросите ссылку снова: /yandex_disk_auth"
        )
...
def exchange_verification_code(code: str, telegram_user_id: int) -> int:
    """Обмен кода со страницы oauth.yandex.ru/verification_code (ручной ввод)."""
    uid = int(telegram_user_id)
    pending = _validate_user_pending(uid)
    if pending is None and not uses_verification_code_flow():
        raise ValueError(
            "Сначала запросите ссылку: /yandex_disk_auth (сессия устарела)."
        )
    if pending is None:
        raise ValueError(
            "Сначала нажмите «Подключить» или выполните /yandex_disk_auth — сессия устарела."
        )
...
def verification_code_instructions() -> str:
    return (
        ...
        "Вставьте его в мини-приложении: Профиль → Яндекс Диск."
    )
...
def get_valid_access_token(telegram_user_id: int) -> str:
    ...
    path = user_token_path(telegram_user_id)
    if not path.exists():
        raise RuntimeError(f"Яндекс Диск не подключён. {_REAUTH_MSG}")
    try:
        raw = path.read_text(encoding="utf-8")
        store = json.loads(raw)
...
def disconnect_user(telegram_user_id: int) -> bool:
    return remove_user_token(telegram_user_id)
