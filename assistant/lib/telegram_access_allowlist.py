...
_allowlist_cache: tuple[float, frozenset[str], frozenset[int]] | None = None
_ALLOWLIST_CACHE_TTL_SEC = 5.0


def _cached_allowlist() -> tuple[frozenset[str], frozenset[int]]:
    global _allowlist_cache
    now = time.time()
    if _allowlist_cache is not None and (now - _allowlist_cache[0]) < _ALLOWLIST_CACHE_TTL_SEC:
        return _allowlist_cache[1], _allowlist_cache[2]
    data = load_allowlist()
    _allowlist_cache = (now, data[0], data[1])
    return data
...
def allowlist_path() -> Path:
    ...
    if raw:
        ...
        if not p.is_absolute():
            p = (ROOT / p).resolve()
        return p
    return (ROOT / "data" / "allowed_telegram_access.json").resolve()
...
def requests_path() -> Path:
    ...
    if raw:
        ...
        if not p.is_absolute():
            p = (ROOT / p).resolve()
        return p
    return (ROOT / "data" / "access_requests.json").resolve()
...
def load_allowlist() -> tuple[frozenset[str], frozenset[int]]:
    path = allowlist_path()
    if not path.is_file():
...
def save_allowlist(usernames: set[str], user_ids: set[int]) -> None:
    path = allowlist_path()
...
def add_approved_user(*, username: str | None, user_id: int) -> None:
    unames, uids = load_allowlist()
    names = set(unames)
    ids = set(uids)
    ids.add(int(user_id))
    if username:
        names.add(username.strip().lstrip("@").lower())
    save_allowlist(names, ids)
    clear_request_state(int(user_id))


def is_extra_allowed(username: str | None, user_id: int) -> bool:
    unames, uids = _cached_allowlist()
...
def is_pending(user_id: int) -> bool:
    pending = _load_requests().get("pending") or {}
...
def consume_action_token(token: str) -> int | None:
    data = _load_requests()
    tokens = dict(data.get("tokens") or {})
    rec = tokens.pop(token, None)
    if not isinstance(rec, dict):
        _save_requests({**data, "tokens": tokens})
        return None
    try:
...
def get_pending_user(user_id: int) -> dict[str, Any] | None:
    pending = (_load_requests().get("pending") or {}).get(str(int(user_id)))
    return dict(pending) if isinstance(pending, dict) else None


def clear_request_state(user_id: int) -> None:
    data = _load_requests()
    pending = dict(data.get("pending") or {})
...
def deny_user(user_id: int) -> None:
    data = _load_requests()
    pending = dict(data.get("pending") or {})
    pending.pop(str(int(user_id)), None)
